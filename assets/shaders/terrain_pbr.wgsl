...
fn get_height(sampled_height: vec4<f32>) -> f32 {
    return sampled_height.x * material.height;
}

fn calculate_normal(uv: vec2<f32>) -> vec3<f32> {
    //     10
    //     |
    // 01--11--21
    //     |
    //     12
    let s01 = get_height(textureSample(height_texture, height_sampler, uv, vec2<i32>(-1, 0)));
    let s21 = get_height(textureSample(height_texture, height_sampler, uv, vec2<i32>(1, 0)));
    let s10 = get_height(textureSample(height_texture, height_sampler, uv, vec2<i32>(0, -1)));
    let s12 = get_height(textureSample(height_texture, height_sampler, uv, vec2<i32>(0, 1)));

    let normal = normalize(vec3<f32>(s21 - s01, 2.0, s12 - s10));
...
fn vertex(vertex: Vertex) -> Fragment {
    ...
    let height = textureSampleLevel(height_texture, height_sampler, uv, 0.0).r;
...
fn fragment(fragment: Fragment) -> [[location(0)]] vec4<f32> {
    ...
    var emissive: vec4<f32> = vec4<f32>(0.0);
    var metallic: f32 = 0.01;
    var perceptual_roughness: f32 = 0.089;
    let roughness = perceptualRoughnessToRoughness(perceptual_roughness);
    var occlusion = 1.0;
    let reflectance = 0.5;

    let world_normal = calculate_normal(fragment.uv);
    ...
    if (is_orthographic) {
        ...
        V = normalize(view.world_position.xyz - fragment.world_position.xyz);
    }
    ...
    let cluster_index = fragment_cluster_index(fragment.frag_coord.xy, view_z, is_orthographic);
    ...
    for (var i: u32 = offset_and_count.offset; i < offset_and_count.offset + offset_and_count.count; i = i + 1u) {
        ...
        if ((mesh.flags & MESH_FLAGS_SHADOW_RECEIVER_BIT) != 0u
            ...
            shadow = fetch_point_shadow(light_id, fragment.world_position, world_normal);
        }
        let light_contrib = point_light(fragment.world_position.xyz, light, roughness, NdotV, N, V, R, F0, diffuse_color);
    ...
    for (var i: u32 = 0u; i < n_directional_lights; i = i + 1u) {
        ...
        if ((mesh.flags & MESH_FLAGS_SHADOW_RECEIVER_BIT) != 0u
            ...
            shadow = fetch_directional_shadow(i, fragment.world_position, world_normal);
        }
